...
draw2d.DecoratedConnection.prototype.toXML = function () {
    ...
    var startX = this.getSource().getAbsoluteX(),
        startY = this.getSource().getAbsoluteY(),
        endX = this.getTarget().getAbsoluteX(),
        endY = this.getTarget().getAbsoluteY();

    var xml = '<Arrow DbId="" Name="' + name + '" ClientId="' + this.lineId + '" LineType="" StartNodeClientId="' + sourceId + '" EndNodeClientId="' + targetId + '" ParentNodeClientId="" BeginPointX="' + startX + '" BeginPointY="' + startY + '" EndPointX="' + endX + '" EndPointY="' + endY + '" />';
...
draw2d.DecoratedConnection.prototype.getContextMenu=function(){
	...
	menu.appendMenuItem(new draw2d.ContextMenuItem("属性", "properties-icon",data,function(x,y)
	{
		...
		var line = data.line;
		var lid = line.getId();
		line.workflow.attrPage.location("Flow/attribute/arrow.htm?lid=" + lid, {
            border: "0",
            scrolling: "no",
            autoSize: false,
            height: "100%",
	...
	menu.appendMenuItem(new draw2d.ContextMenuItem("删除", "remove-icon",data,function(x,y)
	{
		var data = this.getData();
		var line = data.line;
		var lid = line.getId();
