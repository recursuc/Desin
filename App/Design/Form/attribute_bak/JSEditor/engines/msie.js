...
CodePress = {
    ...
    split: function (code, flag) {			//按某种格式分割字符串
        if (flag == 'scroll') {
            this.scrolling = true;
            return code;
        }
    ...
    syntaxHighlight: function (flag) {					//替换高亮显示的文本内的特殊符号
...
Language={};
window.attachEvent('onload', function() { CodePress.initialize('new');});	//在页面加载时绑定初始化函数