...
*/
            ...
            var oCascade = {
                 ...
                 contents:[
                    ...
                    {text:"旅游度假" ,contents: ["CPC","OTATTS","酒店团购","一口价","直销预付","先付返佣"]},
                    {text:"无线事业部" ,contents: ["国内机票","国际机票", "CPC","OTATTS","酒店团购","一口价","直销预付","先付返佣"]},
                    {text:"callcenter" ,contents: ["国内机票","国际机票","CPC","OTATTS","酒店团购","一口价","直销预付","先付返佣"]}
                ],
                events:{
                    "click":null
                },
           ...
           var Container = (function(){
                var bind = function(scope, fn, arg){
                    var args =[];
                    arguments.length > 1 && (args = args.slice.call(arguments, 2));
                    return function(){
                        fn.apply(scope || window, Array.prototype.apply(arguments).concat(args));
                    }
                ...
                Control = function() {
                   return {
                        ...
                        heItem:null,
                        data:null,
                        setData:function(citems){this.citems =citems },
                        setStyle:function(){},
                        make:function(tag, sItem, data){ 
                            this.data = data;
                            this.heItem = new option(sItem, sItem);
                            return this;
                        };
                    } 
                },

                Container = function(data, sId, options){
                    this.obervers = [];
                    this.childControl = [];
                    this.init(data, sId);
                }
                
                Container.prototype = Control();
                extend(Container.prototype, {
                    init:function(data, sId) {
                        this.tag =
                        if(typeof sId == "string"){
                           this.heItem = document.getElementById(sId);
                        }else{
                            if(!data.tag) {
                                this.tag = this.pControl.ctag || "select";
                            }else{
                                var tagNams = data.tag.split(">");
                                this.tag = $.trim(tagNams[0]);
                                if (tagNams.length > 1) {
                                    this.ctag = $.trim(tagNams[1]);
                                }
                            }
                            this.heItem = document.createElement("select");
                            options.container && (options.container.appendChild(heSel));
                        }
                        this.heItem.length = 0;
                        this.createChild(data);
                    },
                    removeChild:function(){

                    },
                    createChild:function(control){
                        for (var i = 0, len = data.length; i < len; i++) {
                            ...
                            this.addChild(this.make(sItem).setCItems(citems));
                        }
                        this.heSel.appendChild(new option(sItem, sItem));

                        $(this.heSel).bind("change",  bind(this, this.notifyHandle, citems));
                        this.childControl.push(control);
                    }
                    attachObserver:function(control){
                        this.obervers.push(control);
                    },
                    notifyHandle:function(evt){
                        this.notity();
                    },
                    notify:function(val){
                        for (var obervers, i = 0, len = this.obervers.length; i < len; i++) {
                            (obervers =this.obervers[i]).update && obervers.update();
                        }
                    }
                };
                return Container;
